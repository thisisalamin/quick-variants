...
<tr class="!border-b !border-gray-200" data-product-id="<?php echo $product->get_id(); ?>">
    ...
    <td class="p-4 align-middle !border text-black !border-gray-200 w-[400px]">
    ...
    <td class="p-4 align-middle !border !border-gray-200 text-center">
        <?php if ($product->is_type('variable')): ?>
            <button class="toggle-variants bg-[#232323] text-white px-4 py-2.5 text-sm hover:bg-white hover:text-black hover:border-black hover:border transition-all duration-300 w-full mx-auto font-bold"
                    data-id="<?php echo $product->get_id(); ?>">
                <?php echo in_array($product->get_id(), $visible_variants) ? 'HIDE VARIANTS' : 'SHOW VARIANTS'; ?>
            </button>
        <?php else: ?>
            <button class="add-to-cart bg-[#232323] text-white px-4 py-2.5 text-sm hover:bg-white hover:text-black hover:border-black hover:border transition-all duration-300 w-full mx-auto font-bold"
                    data-id="<?php echo $product->get_id(); ?>">
                ADD TO CART
            </button>
        <?php endif; ?>
